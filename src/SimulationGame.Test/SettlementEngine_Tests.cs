...
namespace SimulationGame.Test
{
    ...
    public class SettlementEngine_Tests
    {
        ...
        public void Test_AddSettlement_ShouldAddSettlementToSettlementsList()
        {
            ...
            // Assert
            Assert.AreEqual(1, settlementEngine.GetSettlements().Count);
            Assert.AreEqual("Test", settlementEngine.GetSettlements()[0].Name);
            Assert.AreEqual("Test", settlementEngine.GetSettlements()[0].Description);
        }
        ...
        public void Test_RemoveSettlement_RemovesSettlementFromSettlementsList()
        {
            ...
            // Act
            var settlementToRemove = settlementEngine.GetSettlements()[0];
            settlementEngine.RemoveSettlement(settlementToRemove);

            // Assert
            Assert.AreEqual(0, settlementEngine.GetSettlements().Count);
        }
        /*
        [TestMethod]
        ...
        public void Test_GetSettlementsByType_ReturnsCorrectSettlements()
        {
            ...
            // Act
            var result = settlementEngine.GetSettlements().Where(x => x.Type == Enums.SettlementTypes.Village).ToList();
            //var result = settlementEngine.GetSettlementsByType("DifferentType");

            // Assert
            Assert.AreEqual(2, result.Count());
            CollectionAssert.Contains(result, settlementEngine.GetSettlements()[0]);
            CollectionAssert.Contains(result, settlementEngine.GetSettlements()[2]);
        }
        ...
        public void Test_GetAllSettlements_ReturnsAllSettlements()
        {
            ...
            // Act
            var result = settlementEngine.GetSettlements();

            // Assert
            Assert.AreEqual(3, result.Count);
            CollectionAssert.Contains(result, settlementEngine.GetSettlements()[0]);
            CollectionAssert.Contains(result, settlementEngine.GetSettlements()[1]);
            CollectionAssert.Contains(result, settlementEngine.GetSettlements()[2]);
        }
    }
}